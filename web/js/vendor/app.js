...
onload = function () {
    ...
    app.dataSets = [
        ...
        { name: 'Simple (10,000 items)', value: getDataSet(10000) },
    ...
    for (var i = 0; i < app.dataSets.length; i++) {
        app.dataSets[i].value.collectionChanged.addHandler(function () {
            ...
            //app.dataSets[0].value=new wijmo.collections.CollectionView(trueData);
        })
    }
    ...
    // configure the initial view 初始表
    //alert("lll");
    ng.itemsSource = app.dataSets[0].value;
    ng.rowFields.push('Category', 'City');
    ng.valueFields.push('Totalprice', 'Totalprofit');
    ...
    var ngFmt = new wijmo.olap.PivotEngine({
        ...
        itemsSource:app.dataSets[0].value,
        ...
        fields: [
            ...
            { binding: 'totalprofit', header: 'Totalprofit', format: 'n0' },
            { binding: 'totalprofit', header: 'Diff', format: 'p0', showAs: wijmo.olap.ShowAs.DiffRowPct }
        ]
    });
    ngFmt.rowFields.push('Date');
    ngFmt.columnFields.push('Category');
    ngFmt.valueFields.push('Totalprofit', 'Diff');
...
function getSimpleDataSet(cnt) {
   ...
   var res=[{category:"drink",city:"kyf市",date:"2014-12-14",online:"offline",payway:"cash",sex:"female",shopname:"MJA甜品店",totalcost:91.16,totalprice:98.12,totalprofit:6.96},
       {category:"drink",city:"kyf市",date:"2014-12-13",online:"offline",payway:"cash",sex:"female",shopname:"MJA甜品店",totalcost:91.16,totalprice:98.12,totalprofit:6.96},
       {category:"biscuit",city:"kyf市",date:"2014-12-12",online:"offline",payway:"cash",sex:"female",shopname:"MJA甜品店",totalcost:91.16,totalprice:98.12,totalprofit:6.96},
       {category:"cake",city:"kyf市",date:"2014-12-11",online:"offline",payway:"cash",sex:"female",shopname:"MJA甜品店",totalcost:91.16,totalprice:98.12,totalprofit:6.96}];
    var sets;
    $.post(
        ...
        function(data){
            //alert(data);
            sets= $.parseJSON(data);
            res.pop();res.pop();res.pop();res.pop();
            for(var i = 0; i < 5000; i++){
                ...
                res.push({
                    ...
                    city:sets[i].city,
                    date: new Date(ds[0],ds[1],ds[2]),
                    online: sets[i].online,
                    payway: sets[i].payway,
                    sex: sets[i].sex,
                    shopname: sets[i].shopname,
                    totalcost: sets[i].totalcost,
                    totalprice: sets[i].totalprice,
                    totalprofit: sets[i].totalprofit
                });
            }
        }
    );
    trueData=res;
    return new wijmo.collections.CollectionView(res);
}
